...
def add_sphere( points, cx, cy, cz, r, step ):
    p = []
    hold = generate_sphere( p, cx, cy, cz, r, step )
    h = 0
    while h < len(hold):
        add_point(points, hold[h][0], hold[h][1], hold[h][2])
        h += 1
...
def generate_torus( points, cx, cy, cz, r0, r1, step ):
    ...
    while rot < 1:
        ...
        while cir < 1:
            ...
            z = -1 * math.sin(2 * math.pi * rot) * (r0 * math.cos(2 * math.pi * cir) + r1) + cz
            add_point(points, x, y, z)
            cir += step
        rot += step 
    return points
...
def add_torus( points, cx, cy, cz, r0, r1, step ):
    p = []
    hold = generate_torus( p, cx, cy, cz, r0, r1, step )
    h = 0
    while h < len(hold):
        add_point(points, hold[h][0], hold[h][1], hold[h][2])
        h += 1
